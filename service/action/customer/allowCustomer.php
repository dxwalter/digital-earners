<?php 
    ...
    if (empty($customerId)) {
        ...
        $createCustomerObject -> userId = $customerId;
        // open account
        $activateCustomer = $createCustomerObject -> activateInvestor();

        if ($activateCustomer) {
            return $createCustomerObject -> apiStatusCode(200, "Account activated successfully", "true");
        } else {
            return $createCustomerObject -> apiStatusCode(400, "An error occured activating customer's account", "false");
        }
    }
